...
import { getPracticeConfigKey } from "@/lib/storage/keys";
...
export function PracticePageContent() {
  ...
  useEffect(() => {
    ...
    const savedConfig = getSessionItem<PracticeConfig | null>(getPracticeConfigKey(lensKey), null);
    
    if (savedConfig) {
      setSelectedPackIds(savedConfig.selectedPackIds);
      setPackWeights(savedConfig.packWeights);
  ...
  }, [lensKey, packs, lens.state, lens.licenseType, lens.trade]);
  ...
  useEffect(() => {
    ...
    setSessionItem(getPracticeConfigKey(lensKey), config);
  }, [selectedPackIds, packWeights, count, shuffle, lensKey, isHydrated, lens.state, lens.licenseType, lens.trade]);
                            ...
                            <div className="flex items-center gap-3 text-left">
                              ...
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                                  <span>{p.title}</span>
                                  {dueCount > 0 && (
                                    <Badge variant="secondary" className="h-5 px-1.5 text-xs bg-orange-100 text-orange-700 hover:bg-orange-100 border-none">
                                      {dueCount} Due
                                    </Badge>
                                  )}
                                  {newCount > 0 && dueCount === 0 && (
                                    <Badge variant="secondary" className="h-5 px-1.5 text-xs bg-blue-100 text-blue-700 hover:bg-blue-100 border-none">
                                      New
                                    </Badge>
                                  )}
                                </div>
                          ...
                          {isSelected && (
                            <div className="flex items-center gap-2 px-1">
                              ...
                              <input 
                                ...
                                className="w-full h-8 text-sm bg-muted/30 border border-border rounded px-2 focus:outline-none focus:ring-1 focus:ring-primary/50"
                              />
                            </div>
                          )}
                        </div>
